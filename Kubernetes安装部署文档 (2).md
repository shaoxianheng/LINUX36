...
[root@node1 ~]# kubectl describe pods pod-demo -n prod |grep hello
Annotations:  ik8s.io/project: hello

```

### 4.7.1 下载Kubernetes_Advanced_Practical

健康状态检测

```
[root@node1 ~]# git clone https://github.com/iKubernetes/Kubernetes_Advanced_Practical.git
[root@node1 chapter4]# cd /root/Kubernetes_Advanced_Practical/chapter4
[root@node1 chapter4]# cat liveness-exec.yaml
apiVersion: v1
kind: Pod
metadata:
  labels:
...
[root@node1 chapter4]# kubectl apply -f liveness-exec.yaml 
[root@node1 chapter4]# kubectl get pods 
[root@node1 chapter4]# kubectl describe pods liveness-exec 


```

```
[root@node1 chapter4]# cat liveness-http.yaml
apiVersion: v1
kind: Pod
metadata:
  labels:
...
查看http状态
[root@node1 chapter4]# kubectl describe pods liveness-http 


人为删除healthz文件
[root@node1 chapter4]# kubectl exec -it liveness-http -- /bin/sh
/ # 
/ # 
/ # cd /usr/share/nginx/html/
/usr/share/nginx/html # ls
50x.html    healthz     index.html
/usr/share/nginx/html # cat healthz 
Healty
...
```
[root@node1 chapter4]# kubectl explain pods.spec.containers.livenessProbe.httpGet
...
```
[root@node1 chapter4]# kubectl explain pods.spec.containers.readinessProbe
[root@node1 chapter4]# kubectl apply -f readiness-exec.yaml
[root@node1 chapter4]# kubectl get pods -w
[root@node1 chapter4]# kubectl exec readiness-exec -- rm -f /tmp/ready
[root@node1 chapter4]# kubectl get pods -w
[root@node1 chapter4]# kubectl exec readiness-exec -- touch /tmp/ready
...
```
[root@node1 chapter4]# kubectl  apply -f memleak-pod.yaml 
[root@node1 chapter4]# kubectl describe pod memleak-pod   
...
```
[root@node1 chapter5]# kubectl apply -f rs-example.yaml 
[root@node1 chapter5]# kubectl get rs
[root@node1 chapter5]#  kubectl describe rs myapp-rs
[root@node1 chapter5]# kubectl get pods   -o wide
...
```
[root@node1 chapter5]# kubectl delete all -n prod
[root@node1 chapter5]# cat deploy-nginx.yaml
apiVersion: apps/v1
kind: Deployment
metadata:
  name: myapp
...
[root@node1 chapter5]# kubectl get deploy -n prod
[root@node1 chapter5]# kubectl get -n prod rs
[root@node1 chapter5]# kubectl get -n prod pods

更新depoly版本，编辑deploy-nginx.yaml 文件修改下面的字段
 image: ikubernetes/myapp:v2
 [root@node1 chapter5]# kubectl apply -f deploy-nginx.yaml 
 [root@node1 chapter5]# kubectl get rs -n prod -o wide
...
```
[root@node1 chapter5]# kubectl apply -f deploy-nginx.yaml --record
[root@node1 chapter5]# kubectl describe -n prod deploy myapp
...
[root@node1 ~]# kubectl set image deployment myapp myapp=ikubernetes/myapp:v3 -n prod
deployment.apps/myapp image updated
[root@node1 ~]# kubectl get rs -n prod -o wide
...
```
[root@node1 ~]# kubectl rollout -h 
[root@node1 ~]# kubectl rollout history deployment/myapp -n prod
[root@node1 ~]# kubectl rollout undo --to-revision=2 deployment/myapp -n prod
[root@node1 chapter5]# kubectl rollout status deployment/myapp -n prod
[root@node1 ~]# kubectl rollout history deployment/myapp -n prod
[root@node1 ~]# kubectl get rs -n prod -o wide
...
```
[root@node1 chapter5]# kubectl get pods -n prod -w
[root@node1 ~]# kubectl set image deployment myapp -n prod myapp=ikubernetes/myapp:v4 && kubectl rollout pause deployment/myapp -n prod
[root@node1 chapter5]# kubectl rollout history deployment/myapp -n prod
[root@node1 ~]# kubectl get pods -n prod 
...
```
[root@node1 chapter5]# kubectl apply -f filebeat-ds.yaml
添加上namespace：prod
[root@node1 chapter5]# kubectl  get pods -n prod --show-labels
[root@node1 chapter5]# kubectl  get pods -n prod -l app=filebeat
[root@node1 chapter5]# kubectl  get pods -n prod -l app=filebeat -o wide
...
```
[root@node1 chapter5]# kubectl describe node node1
...
```
[root@node1 chapter5]# kubectl  get pods -n prod -l app=filebeat -w

[root@node1 chapter5]# kubectl set image ds/filebeat-ds filebeat=ikubernetes/filebeat:5.6.6-alpine -n prod
[root@node1 chapter5]# kubectl get ds -n prod -o wide
[root@node1 chapter5]# kubectl describe pods filebeat-ds-45bkf -n prod
...
```
[root@node1 chapter5]# kubectl delete ds filebeat-ds -n prod
...
```
[root@node1 chapter5]# kubectl get nodes --show-labels

[root@node1 chapter5]# cat filebeat-ds.yaml 
apiVersion: apps/v1
kind: DaemonSet
metadata:
  name: filebeat-ds
...
[root@node1 chapter5]# kubectl label node node2 logcollecting="on"
[root@node1 chapter5]# kubectl get pods -n prod
[root@node1 chapter5]# kubectl label node node3 logcollecting="on"
node/node3 labeled
[root@node1 chapter5]# kubectl get pods -n prod -o wide

[root@node1 chapter5]# kubectl get ds -n kube-system
...
```
[root@node1 chapter5]# cat job-example.yaml 
apiVersion: batch/v1
kind: Job
metadata:
  name: job-example
...
spec:
  template:
    ...
    spec:
      ...
      restartPolicy: Never
[root@node1 chapter5]# kubectl get job -n prod
[root@node1 chapter5]# kubectl get pods -n prod

```

```
[root@node1 chapter5]# cat job-multi.yaml
apiVersion: batch/v1
kind: Job
metadata:
  name: job-multi
...
spec:
  ...
  template:
    ...
    spec:
      ...
      restartPolicy: Never
[root@node1 chapter5]# kubectl create ns test
namespace/test created
[root@node1 chapter5]# kubectl apply -f job-multi.yaml 
job.batch/test created
[root@node1 chapter5]# kubectl get pods -n test -w

```

```
[root@node1 chapter5]# kubectl delete job job-multi -n test
[root@node1 chapter5]# cat job-multi.yaml
apiVersion: batch/v1
kind: Job
metadata:
...
```
[root@node1 chapter5]# kubectl delete jobs job-multi -n test 
...
[root@node1 ~]# kubectl apply -f myapp-svc.yaml 
[root@node1 ~]# kubectl get svc -n prod
[root@node1 ~]# curl 10.106.162.253
Hello MyApp | Version: v4 | <a href="hostname.html">Pod Name</a>
[root@node1 ~]# kubectl describe svc myapp -n prod
[root@node1 ~]# kubectl get endpoints -n prod
[root@node1 ~]# kubectl get endpoints -o yaml -n prod
...
[root@node1 ~]# kubectl get svc -n prod
浏览器访问：
http://192.168.0.114:30080/
```

指定endpoints

```
[root@node1 ~]# kubectl explain  endpoints.subsets.addresses
[root@node1 ~]# kubectl delete -f myapp-svc.yaml
[root@node1 ~]# cat myapp-svc.yaml
apiVersion: v1
kind: Service
metadata: 
